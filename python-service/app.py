...
yf.set_proxy(None)  # optional: remove proxy if set
yf.pdr_override()   # ensure yfinance uses pandas_datareader style
yf.download(session=session, progress=False)  # ensure session is used globally

# ----------------------------
# Simple in-memory cache
# ----------------------------
CACHE = {}  # key: (tickers tuple, period), value: (timestamp, result)
...
def optimise():
    ...
    # Save to cache
    CACHE[cache_key] = (datetime.utcnow(), clean_json(result))
